...
@Component
class MessagesContactProcessor(
        private val service: MessageService
) : AbstractAttributeTagProcessor(TemplateMode.HTML, "th", null, false, "jspbb-messages-contact", true, 1000, true) {
    override fun doProcess(context: ITemplateContext, tag: IProcessableElementTag, attributeName: AttributeName, attributeValue: String, structureHandler: IElementTagStructureHandler) {
        ...
        val contactUserId: Long = ThymeleafUtils.getLong(context, "contactUserId") ?: throw RuntimeException("contactUserId cannot be null")
        ...
        if (pageable) {
            ...
            structureHandler.setLocalVariable(attributeValue, service.selectContactUserIdPage(userId, contactUserId, page, pageSize))
        } else {
            val offset = ThymeleafUtils.getOffset(context)
            val limit = ThymeleafUtils.getLimit(context)
            structureHandler.setLocalVariable(attributeValue, service.selectContactUserIdList(userId, contactUserId, offset, limit))
        }
    }
}